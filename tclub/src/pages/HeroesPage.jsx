...
const HeroesPage = () => {
  const { currentPage, totalPages, currentItems, handlePageChange } = usePagination(cardData, 8);

  return (
    <>
      <div>
        <Banner title="OUR HEROES" image={heroesImage} width={315} height={342}  roundedBr="rounded-none"/>
      </div>

      <div className="flex items-center justify-center w-full">
        <span className="bg-[var(--badge-bg)] text-[14px] leading-[27px] tracking-[0] font-semibold text-[var(--text-primary)] py-[10px] px-[20px] w-[230px] text-center rounded-b-2xl">
          Stories Of The Heroes
        </span>
      </div>
      ...
      <div className="container mx-auto max-w-9xl lg:pt-[80px] md:pt-[60px] pt-[40px] pb-[60px] lg:container max-w-full md:max-w-9xl md:mx-0 lg:mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-2 md:grid-cols-1 gap-6 p-4">
          {currentItems.map((card) => (
            <div
              key={card.id}
              className="bg-white p-[30px] rounded-4xl border border-[var(--border-light)] cursor-pointer"
            >
              {/* Rounded Image */}
              <div className="flex items-center gap-[20px]">
                <div className="w-20 h-20 rounded-full overflow-hidden">
                ...
                <div className="font-semibold text-[var(--text-primary)] flex flex-col items-baseline gap-2">
                  <div className="lg:text-[25px] text-[18px] font-semibold">{card.name}</div>
                  <div className="flex items-center justify-center gap-2 bg-[var(--vip-bg)] text-white text-xs font-bold py-[6px] px-[10px] rounded-2xl">
                    <img src={crownImg} alt="Crown" className="w-4 h-4 object-contain" />
                    <span>VIP</span>
                  </div>
                </div>
              ...
              {/* Divider */}
              <div className="flex items-center w-full my-[30px] relative before:content-[''] before:absolute before:left-0 before:right-0 before:top-1/2 before:h-[1px] before:bg-[var(--border-light)]">
                <span className="bg-[var(--badge-bg)] text-[14px] font-semibold text-[var(--text-primary)] py-[10px] px-[20px] w-[230px] text-center z-10 rounded-[5px]">
                  Stories Of The Heroes
                </span>
              </div>

              {/* Description */}
              <p className="text-[14px] text-[var(--text-secondary)] leading-[22px] tracking-normal">
                {card.description.includes('Read More.') ? (
                  <>
                    {card.description.replace('...Read More.', '')}
                    <NavLink to={`/details`} className="text-[var(--link-color)] font-medium hover:underline">
                      ...Read More.
                    </NavLink>
                  </>
                ) : (
