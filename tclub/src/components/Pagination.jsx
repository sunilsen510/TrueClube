...
const Pagination = ({ currentPage, totalPages, onPageChange }) => {
  ...
  return (
    <>
      <div className="relative flex justify-center cursor-pointer">
        <div className="flex items-center gap-2 rounded-full border border-[var(--divider-color)] w-fit px-4 mt-[60px] py-1">
          
          {/* Previous Button */}
          <button
            onClick={() => handlePageChange(currentPage - 1)}
            disabled={currentPage === 1}
            className="px-3 py-1 text-[15px] font-normal text-[var(--text-primary)] disabled:opacity-50 cursor-pointer"
          >
            <img
              src={leftArrow}
              alt="Previous"
              className="hover:text-[var(--text-link)] hover:brightness-0 cursor-pointer transition duration-200 w-[13px] h-[12px]"
            />
          </button>
          ...
          {[...Array(totalPages)].map((_, idx) => {
            ...
            return (
              <div className="flex flex-col items-center" key={pageNumber}>
                <button
                  onClick={() => handlePageChange(pageNumber)}
                  className={`px-3 py-1 rounded text-[15px] font-normal border transition cursor-pointer ${
                    currentPage === pageNumber
                      ? 'bg-transparent text-[var(--text-primary)] border-transparent'
                      : 'bg-white text-[var(--text-secondary)] border-transparent'
                  }`}
                >
                  {formatted}
                </button>
                ...
                {currentPage === pageNumber && (
                  <span className="w-[31px] h-[2px] bg-[var(--btn-primary-bg)] rounded-full"></span>
                )}
              </div>
            );
          })}
          ...
          >
            <img
              src={rightArrow}
              alt="Next"
              className="w-[13px] h-[12px] hover:text-[var(--text-link)] hover:brightness-0 cursor-pointer transition duration-200"
            />
          </button>
