...
const BlogCard = ({ blog, locationImg, calendarImg, userImg, cardImg, cardTitle }) => {
  return (
    <div className="p-[15px] gap-[25px] w-full bg-white border border-gray-200 rounded-xl overflow-hidden flex flex-col shadow-sm">
      ...
      <div className="flex flex-col gap-[16px]">
        {/* Title */}
        <h3 className="font-semibold text-[20px] tracking-[0] text-[var(--text-primary)]">
          {cardTitle}
        </h3>

        {/* Info (Admin & Date) */}
        <div className="flex items-center gap-[14px] text-sm text-[var(--text-muted)]">
          <div className="flex items-center gap-2">
            <img src={userImg} alt="user" className="w-4 h-4" />
        ...
        {/* Description */}
        {blog.description && (
          <p className="text-[14px] text-[var(--text-secondary)] font-normal">
            {blog.description}
          </p>
        )}
